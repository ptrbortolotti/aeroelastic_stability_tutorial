...
folder_name = os.path.join(MyDir, 'outputs/bar_urc/stability_bar_urcs_rotor_fixrpm')
...
try:
    ...
    fig, axes, figName =  lin.plotCampbellDataFile(modeID_file, 'rpm', ylim=None, to_csv=True)
except:
    # Find lin files, perform MBC, and try to identify modes. A csv file is written with the mode IDs.
    OP, Freq, Damp, UnMapped, ModeData, modeID_file = lin.postproCampbell(fstFiles, BladeLen, TowerLen, removeTwrAzimuth=True)
    fig, axes, figName =  lin.plotCampbellDataFile(modeID_file, 'rpm', ylim=None, to_csv=True)
